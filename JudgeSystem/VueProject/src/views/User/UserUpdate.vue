        ...
        <el-form style="width: 40%" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item label="角色权限" prop="role_id">
                <el-select v-model="ruleForm.role_id" placeholder="请选择角色权限">
                    ...
                    <el-option label="评审负责人" value="2"></el-option>
                    <el-option label="评审老师" value="3"></el-option>
...
<script>
    export default {
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put('http://localhost:8010/user/update',this.ruleForm).then(function (resp) {
                            if (resp.data==1) {
        ...
        created() {
            const _this=this
            axios.get('http://localhost:8010/user/findById/'+ this.$route.query.id).then(function(resp) {
